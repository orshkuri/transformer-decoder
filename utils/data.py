...
class CharTokenizer:
    ...
    def save(self, path: str) -> None:
        ...
        tokenizer_state = {
            ...
            'tokens': list(self.tokens),  # Convert set to list for JSON serialization
            'vocab': self.vocab,
            'stoi': self.stoi
        }
    ...
    def load(path: str) -> CharTokenizer:
        ...
        tokenizer.symbols = tokenizer_state['symbols']
        tokenizer.tokens = set(tokenizer_state['tokens'])  # Convert list back to set
...
def load_data(path: str, load_file_type: str = None) -> [CharTokenizer, list[list[int]]]:
    if load_file_type:
        tokenizer = CharTokenizer.load(f"tokenizer_{load_file_type}.json")
    else:
        tokenizer = CharTokenizer()
